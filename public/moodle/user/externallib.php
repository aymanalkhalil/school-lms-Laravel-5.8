...
class ys_user_external extends external_api {
     ...
     * @since Moodle 2.2
     */
    public static function  account_creation_parameters() {
        global $CFG;
                ...
                'users' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            'idnumber' =>
                                new external_value(core_user::get_property_type('idnumber'), 'An arbitrary ID code number perhaps from the institution',VALUE_OPTIONAL),
                                'phone1' =>
                                new external_value(core_user::get_property_type('phone1'), 'phone number'),
                            'lang' =>
     ...
     * @since Moodle 2.2
     */
    public static function account_creation($users) {
        global $CFG, $DB;
        require_once($CFG->dirroot."/lib/weblib.php");
        require_once($CFG->dirroot."/user/lib.php");
        require_once($CFG->dirroot."/user/editlib.php");
        ...
        // Do basic automatic PARAM checks on incoming data, using params description.
        // If any problems are found then exceptions are thrown with helpful error messages.
        $params = self::validate_parameters(self::account_creation_parameters(), array('users' => $users));
        ...
        foreach ($params['users'] as $user) {
          ...
          //      throw new invalid_parameter_exception('Useridnumber already exists: '.$user['idnumber']);
          //  }
     ...
     * @since Moodle 2.2
     */
    public static function account_creation_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'       => new external_value(core_user::get_property_type('id'), 'user id'),
 ...
 public static function  password_update_parameters() {
        global $CFG;
            ...
            array(
                'users' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'username' =>
                                new external_value(core_user::get_property_type('username'), '   '),
                            'password' =>
                                new external_value(core_user::get_property_type('password'), 'Plain text password consisting of any characters'))))));}

 public static function  password_update($users) {
         global $CFG, $DB;
        require_once($CFG->dirroot."/lib/weblib.php");
        require_once($CFG->dirroot."/user/lib.php");
        require_once($CFG->dirroot."/user/editlib.php");
        require_once($CFG->dirroot."/user/profile/lib.php");
 $params = self::validate_parameters(self::password_update_parameters(), array('users' => $users));
       foreach ($params['users'] as $user) {
            // Make sure that the username doesn't already exist.
          
            
             if ($DB->record_exists('user', array('username' => $user['username'] ))) {
                user_update_password($user['password'],$user['username']);
            }
...
public static function password_update_returns() {
    return null;
